...
async function loadPolymarket(){
  ...
  if(c&&c.q===q&&now-c.ts<300000){
    polyMarkets=c.markets||[];
    polyTeamPrices=c.prices||{};
    polyMatchMarkets=c.matchMarkets||{};
    return;
  }
  try{
    let url=`${POLY_URL}?closed=false&limit=100&search=${encodeURIComponent(q)}`;
    let res=await fetch(url);
    if(!res.ok)throw new Error(`HTTP ${res.status}`);
    let markets=await res.json();
    if(!Array.isArray(markets))markets=markets.data||[];
    ...
    for(const m of polyMarkets){
      ...
      outcomes.forEach((out,i)=>{
        let c=code(out),p=priceNum(priceArr[i]);
        if(c&&p!==null)prices[c]=Math.max(prices[c]||0,p)
      })
    }
    ...
    if(tournament){
      for(const gm of tournament.groupMatches){
        ...
        if(found) matchMap[gm.id] = {
          slug: found.market.slug || "",
          question: found.market.question || found.market.title || "",
          home: found.prices[gm.home],
          draw: found.prices.DRAW,
          away: found.prices[gm.away]
        };
      }
    }
    polyMatchMarkets = matchMap;

    localStorage.setItem(LS.polyCache,JSON.stringify({ts:now,q,markets:polyMarkets,prices:polyTeamPrices,matchMarkets:polyMatchMarkets}))
